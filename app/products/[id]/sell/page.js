    ...
    <div className="min-h-screen bg-[#f4f1f9] text-[#2b2438]">
      ...
      <header className="sticky top-0 z-10 bg-white/90 backdrop-blur border-b border-[#e7e2f3] px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-medium text-[#4b4560]">Vendre</span>
        </div>
      ...
      <main className="max-w-2xl mx-auto px-4 py-10">
        ...
        <div className="mb-8">
          <h2 className="text-2xl font-serif italic text-[#2b2438]">Vendre « {product.name} »</h2>
          <p className="text-[#948da8] text-sm mt-1">Coût d'achat : {(product.purchase_price + product.purchase_fees).toFixed(2)}€</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm shadow-[#2b2438]/5 p-6 md:p-8 flex flex-col gap-6">
          ...
          {form.sale_price && (
            <div className={`rounded-xl px-4 py-3 flex items-center justify-between border ${
              margin >= 0 ? 'bg-[#4f8f6e]/10 border-[#4f8f6e]/20' : 'bg-[#c14f4a]/10 border-[#c14f4a]/20'
            }`}>
              <div>
                <p className={`text-xs font-semibold uppercase tracking-wider ${margin >= 0 ? 'text-[#4f8f6e]' : 'text-[#c14f4a]'}`}>Marge nette</p>
                <p className={`text-xs mt-0.5 ${margin >= 0 ? 'text-[#4f8f6e]/70' : 'text-[#c14f4a]/70'}`}>Vente − coût d'achat − frais</p>
              </div>
              <span className={`text-xl font-serif ${margin >= 0 ? 'text-[#4f8f6e]' : 'text-[#c14f4a]'}`}>{margin}€</span>
            </div>
          )}
